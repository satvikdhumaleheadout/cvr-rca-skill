...
- **Tab 2 — Paid Performance Audit**: read `perf_audit_report.md` and convert
  ...
  every table cell, every paragraph, every word.** No summarization. No
  restructuring. No CVR-RCA-style callout chrome wrapped around perf-audit
  content. The Tab 2 looks like a polished markdown render of the audit, not
  a parallel CVR-RCA mini-report. The perf-audit-skill is the source of truth
  for its own structure; CVR-RCA's job is to display it, not to re-format it.

**Important — do NOT use `perf_audit_report.html` even if it exists.** If the
perf-audit-skill emits a sibling `.html` file, ignore it. That file may
restructure or summarize the markdown, which we don't want. The canonical
source is the `.md` file. Tab 2 is always rendered from `.md`, never from
`.html`.
...
would only happen if perf-audit-skill introduces a markdown construct not
covered by the conversion mapping), fall back to embedding the raw markdown
text directly inside a `<pre class="md-raw">` block in Tab 2. Better to show
raw markdown than a paraphrased HTML — fidelity beats polish.
